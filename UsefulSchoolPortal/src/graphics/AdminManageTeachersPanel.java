...
public class AdminManageTeachersPanel extends JPanel {
	...
	public AdminManageTeachersPanel() {
	...
	private void prepareCenterPanel() {
		...
		JLabel selectTeacher = new JLabel("Select Teacher");
		selectTeacher.setFont(GraphicsConstants.FONT_ROBOTO_B50);
		
		teacherList = new JComboBox<String>();
		teacherList.setFont(GraphicsConstants.FONT_ROBOTO_B30);
		...
		enterEdits.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				Teacher t = teachers.get(teacherList.getSelectedIndex());
		...
		sl.putConstraint(SpringLayout.WEST, teacherList, 100, SpringLayout.EAST, selectTeacher);
		sl.putConstraint(SpringLayout.NORTH, teacherList, 100, SpringLayout.NORTH, centerPanel);
	...
	public void refreshComboBox() {
		...
		teachers = DataManagement.getCurrentSchoolTeachers();

		if (teachers != null) {
			for (Teacher t : teachers) {
				teacherList.addItem(t.getFirstName() + " " + t.getLastName());
			}
		}
	}
